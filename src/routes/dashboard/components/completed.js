      ...
      <ResponsiveContainer minHeight={360}>
        <AreaChart data={data}>
          ...
          <Tooltip
            ...
            content={(content) => {
              const list = conent.payload.map((item, key) => <li key={key} className={styles.tipitem}><span className={styles.radiusdot} style={{ background: item.color }} />{`${item.name}:${item.value}`}</li>)
